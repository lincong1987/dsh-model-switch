...
This plugin requires DeepSeek Harness **0.1.2-alpha.4** or later. DSH has no `dsh update` command — install the version you want the same way you originally installed it.
...
As of 2026-09-02, `latest` and `next` point to `0.1.1-rc.2`, while the `alpha` tag points to `0.1.2-alpha.4` (this plugin's current target). Pin the exact version (`@0.1.2-alpha.4`) when reproducibility matters.
...
Requires DeepSeek Harness **0.1.2-alpha.4** or later. Settings are stored in the host settings document through the plugin's registered `model-switch` namespace.
