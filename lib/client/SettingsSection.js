...
export function SettingsSection(props) {
    ...
    return (_jsxs("div", { className: css.section, children: [_jsxs("div", { className: css.titleRow, children: [_jsx(IconModelSwitch, { className: css.titleIcon, size: 18 }), _jsx("h2", { className: css.title, children: t('title') })] }), _jsx("p", { className: css.intro, children: t('intro') }), error !== null || snap.error !== null
                ? (_jsxs("p", { className: css.error, children: [t('saveError'), ": ", snap.error === 'unavailable' ? t('unavailable') : (error ?? snap.error)] }))
                : null, _jsx(RouteBlock, { title: t('subagentTitle'), hint: t('subagentHint'), route: subagent, sessionId: sessionId, access: access, t: t, busy: busy || snap.status === 'loading', onChange: (next) => { writeRoute('subagent', next); } }), _jsx(RouteBlock, { title: t('planTitle'), hint: t('planHint'), route: planExecute, sessionId: sessionId, access: access, t: t, busy: busy || snap.status === 'loading', onChange: (next) => { writeRoute('planExecute', next); } })] }));
}
